...
require_once 'includes/email-templates/email-sender.php';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($yearOfStudy) || !in_array($yearOfStudy, [1, 2, 3, 4])) {
        $errors[] = "Valid year of study is required";
    }
    ...
    if ($stmt->execute()) {
        ...
        try {
            $welcome_data = [
                ...
                'email' => $email,
                'role' => 'student',
                // You can add verification link if you implement email verification
                // 'verification_link' => 'https://yourdomain.com/verify.php?token=' . generateToken()
            ];
            
            // Send welcome email (log error if fails but don't stop registration)
            if (!sendWelcomeEmail($welcome_data)) {
        ...
        $_SESSION['registration_success'] = "Account created successfully. You can now login. A welcome email has been sent to your email address.";
