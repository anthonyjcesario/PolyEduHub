...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        ...
        // Attempt to send welcome email (don't block registration if email fails)
        try {
            sendWelcomeEmail($welcome_data);
        } catch (Exception $e) {
            // Log the error but continue with registration
            error_log("Failed to send admin welcome email to $email: " . $e->getMessage());
        }
